...
$dis = mysqli_query($conn, "SELECT p.*, pt.*, p.id as ide
 ...
 {
if(isset($_POST['idpro']) and $_POST['idpro']>0 and $nins[$dis1['ide']]>0)
$chekme="checked"; else $chekme="";
$filteri .='<input value="'.$dis1['ide'].'-'.$fis1['ide'].'" class="fstavke" type="radio" name="filt'.$i.'" id="ime'.$i.$j.'" '.$chekme.'  /> <label for="ime'.$i.$j.'">'.$dis1['naziv'].'</label><br />';
