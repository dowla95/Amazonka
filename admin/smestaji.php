...
?>
<div class='naslov_smestaj'><h1>Upisani tekstovi za stranicu i podstranice <span style='color:#444;'><?php echo $ova1["naziv$amlang"]?></b></h1></div>
</td>
 <td align='right'><a href="<?php echo $patHA?>/index.php?base=admin&page=page_add_content&tip=id_page&idp=<?php echo $_GET['idp']?>"><span style='padding:6px 10px;font-size:15px;'>UPIŠI NOVI TEKST</span></a>
...
$br_upisa=mysqli_num_rows(mysqli_query($conn, "SELECT * FROM  smestaj WHERE $andidpS id>0  $plun"));

$pagedResults = new Paginated($br_upisa, $ByPage1, $page_cur);
$str=$pagedResults->fetchPagedRow();
$pagedResults->setLayout(new DoubleBarLayout());

$fi=mysqli_query($conn, "SELECT * FROM smestaj WHERE $andidpS id>0 $plun ORDER BY  $orderby LIMIT $str,$ByPage1");
...
<!--<tr>
  ...
  <td>Stranica</td>
    ...
    <td>PDF fajlovi</td>
      ...
      <td>Istakni</td>    
   <td>Akt</td>   
     <!--<td>Prikaži koment</td>-->
     <!--<td>Dozvoli koment</td>-->
...
$msg_id=$og1['id']; 
$zzL=mysqli_query($conn, "SELECT * FROM smestaj_lang  WHERE id_text=$og1[id] AND lang='$firstlang'");
...
<td>
<?php 
$cg=mysqli_query($conn, "SELECT * FROM pageL WHERE id='$og1[id_page]' AND lang='$firstlang'");
...
<td>
<?php 
$hg=mysqli_query($conn, "SELECT * FROM files WHERE idupisa='$og1[id]' AND tip=0");

echo "<a href='$patHA/index.php?base=admin&page=filesadd&idupisa=$og1[id]&tip=0'>";
echo mysqli_num_rows($hg);
echo "</a>";
?>
</td>
...
?>
<td>
<a class="iframes1" href="<?php echo $patHA?>/prikaz_text.php?idu=<?php echo $og1[id]?>&tip=text">istakni u model stranice</a>
...
<td>
<a href="<?php echo $patHA?>/index.php?base=admin&page=page_edit_content&id=<?php echo $og1[id]?>&tip=id_page&idp=<?php echo $idp?>" class="edit_oglas"><img src="<?php echo $patHA?>/images/b_edit.png" /></a>&nbsp;&nbsp;
