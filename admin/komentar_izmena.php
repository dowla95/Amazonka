...
$tr=mysqli_query($conn, "SELECT * FROM comments WHERE id=$id");
...
if(!mysqli_query($conn, "UPDATE comments SET naslov=".safe(strip_tags($ime)).", email=".safe(strip_tags($email)).", opis=".safe(strip_tags($Text))."  WHERE id=$_GET[id]")) echo mysqli_error();
$msgr_ok="Komentar je izmenjen!";
										...
										<form action="#komentar" id="commentform" method="post">
											...
											<div class="field-row">
												<label for="contact_name">Ime <span>(obavezno)</span></label>
											   	<input type="text" name="ime" id="contact_name" class="text_input" value="<?php echo $_POST[ime]?$_POST[ime]:$tr1[naslov]?>" />
											</div>

											<div class="field-row">
												<label for="email">Email <span>(obavezno)</span></label>
												<input type="text" name="email" value="<?php echo $_POST[email]?$_POST[email]:$tr1[email]?>" id="email" class="text_input"  />		
											</div>
											...
											<div class="field-row">
												...
												<textarea name="tekst" id="message_text" class="text_input" cols="60" rows="9"><?php echo $_POST[tekst]?$_POST[tekst]:$tr1[opis]?></textarea>
											</div>
...
<input type="hidden" name="id_parent" value="<?php echo $id_parent?>" />
											<input class="button2" type="submit" value="Izmenite komentar" id="submit" name="upisi_komentar">
