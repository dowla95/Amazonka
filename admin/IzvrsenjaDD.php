...
$rtz = mysqli_query($conn, "SELECT p.*, pt.*
        ...
        WHERE pt.lang='$firstlang' AND p.id_cat=".strip_tags($_GET['id_cat'])."  AND NOT p.id=".strip_tags($_GET['id'])."  AND pt.ulink='".replace_implode1($_POST["naziv$firstlang"])."' AND p.nivo=$nivo");
...
$rtz = mysqli_query($conn, "SELECT p.*, pt.*
        FROM stavke p
        INNER JOIN stavkel pt ON p.id = pt.id_page        
        WHERE pt.lang='$firstlang' AND p.id_cat=".strip_tags($_GET['id_cat'])." AND pt.ulink='".replace_implode1($_POST["naziv$firstlang"])."' AND p.nivo=$nivo");
   $rtz1=mysqli_fetch_array($rtz);
...
mysqli_query($conn, "INSERT INTO pro SET $nazivisa");
$idzad=mysqli_insert_id($conn);
if($idzad>0)
{
$la=mysqli_query($conn, "SELECT * FROM language WHERE akt='Y' order by pozicija ASC");
...
//, kategorija='$_POST[kategorija]'
$nazivisa ="akcijatraje='$_POST[akcijatraje]', kupindo='$_POST[kupid]', link='$_POST[link]', canurl='$_POST[canurl]', video='$video', lager='$lager', cena='$_POST[cena]', cena1='$_POST[cena1]', slika='$slika', slika1='$slika1', $pozicija, $akt, $akci, $novic, $nasc, $izac, $vegani, $tipi1, $tipi2, $tipi3, $tipi4, $izuk, time='".time()."', brend='$_POST[brendovi]'";
if(!mysqli_query($conn, "UPDATE pro SET $nazivisa WHERE id=$id_get")) echo mysqli_error(); 
