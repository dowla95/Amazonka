...
if($pate1['id']>0)
{
$titles=$pate1['tidtle']?$pate1['title']:$pate1['naslov'];
$keywords=$pate1["keywords"];
$descripts=$pate1["description"];
$and_tekst=" AND p.id=$pate1[id]";
...
if(empty($pate1['id'])){
$mitos=explode("-",$nodom_ex_r[0]);
$idpro=end($mitos);
if($idpro>0) {
$dztz = mysqli_query($conn, "SELECT p.*, pt.*, p.id as ide
        FROM pro p
        INNER JOIN prol pt ON p.id = pt.id_text
        WHERE pt.lang='$lang' AND p.akt=1 AND p.id=$idpro");

        //AND ulink=".safe($nodom_ex_r[0])."
...
if(isset($sarray['edit']) and $sarray['edit']>0) {
$fed=mysqli_query($conn, "SELECT *, p.id as id FROM  pro p
INNER JOIN prol pl ON pl.id_text=p.id
WHERE p.tip=5 AND p.akt=1 AND prodavac=$_SESSION[userid] AND p.id=".safe($sarray['edit']));
$fed1=mysqli_fetch_assoc($fed);

$titles .=" - Izmena ".$fed1['naslov'];
}
...
/*
$m=mysqli_query($conn, "SELECT * FROM pro");
while($m1=mysqli_fetch_assoc($m)){
$a=mysqli_num_rows(mysqli_query($conn, "SELECT * FROM kat_pro WHERE kat=$m1[kategorija] AND pro=$m1[id]"));
if($a<1)
mysqli_query($conn, "INSERT INTO kat_pro SET kat=$m1[kategorija], pro=$m1[id]");
}*/
...
/*$ptnja = mysqli_query ($conn, "SELECT * FROM stavkel WHERE ulink='$base_arr_r[1]'");
$ptnja1 = mysqli_fetch_array($ptnja);
$ptnja2 = mysqli_query ($conn, "SELECT * FROM stavkel WHERE ulink='$base_arr_r[2]'");
$ptnja3 = mysqli_fetch_array($ptnja2);*/
...
/*$inner_plus .="INNER JOIN pro_filt pf ON p.id = pf.id_pro";
$iFilter="AND pf.id_filt IN (".implode(",", $filtNiz).")";
$iFilter1=", count(pf.id_filt) as ukup";
*/
}
...
$zasum=mysqli_query($conn, "SELECT p.*, pt.*, p.id as ide
        FROM pro p
        INNER JOIN prol pt ON p.id = pt.id_text
        WHERE pt.lang='$lang' AND p.prodato=0 AND p.akt=1 AND p.tip=5 $koniPlus $inner_plus1$ibrendis  GROUP BY p.id");
        //$br_upisa=mysqli_num_rows($zasum);
        $br_upisa=0;
        $minMax=array();
...
$mac=$mi1['macena'];
*/
/* ovo iznad bilo je u oprema-load.php */
