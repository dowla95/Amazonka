...
$fari=mysqli_query($conn, "SELECT id_priv FROM porudzbine WHERE id_priv=$ari1[id]");
$fari1=@mysqli_num_rows($fari);
...
if(strlen($ari1['trackid'])>3  and $diff>0.06 and $fari1==0)
{
$xml_str="<CC5Request>
            <Name>BizNet</Name>
...
/*if(getIP()=="212.200.65.80")
echo "UPDATE privremena SET payamentid='$PayID', transactionid='$TransID', status=$mdStatus, ProcReturnCode=".safe($ProcReturnCode).", auth='$AutCode', resultcode='$ResCode', trantype='PreAuth', postdate='$PosDate', poslat_email='1', proveren=1  WHERE trackid='$TrckID'";*/
posaljiK("uspeh",2);
...
$fariz=mysqli_query($conn, "SELECT id_priv FROM porudzbine WHERE id_priv=$ari1[id]");
$fari2=@mysqli_num_rows($fariz);
if($fari2>0)
mysqli_query($conn, "UPDATE privremena SET poslat_email='1', proveren=1  WHERE id=$ari1[id]");
}
