...
$ari=mysqli_query($conn, "SELECT * FROM privremena WHERE status=0 AND poslat_email=0 ORDER BY id DESC LIMIT 1");
...
if(strlen($ari1['trackid'])>3  and $diff>0.3)
{
$xml_str="<CC5Request>
            <Name>BizNet</Name>
...
if($pri1['id']>0)
{
if(!mysqli_query($conn, "INSERT INTO porudzbine SET user_id=$pri1[usid], ime=".safe($pri1[ime]).", adresa=".safe($pri1[adresa]).", posta=".safe($posta).", grad=".safe($pri1[grad]).", telefon=".safe($pri1[telefon]).", email=".safe($pri1[mejl]).", pib=".safe($pri1[pib]).", nacin_placanja=4, poruka=".safe($pri1['poruka']).", iznos=".safe($pri1['iznos']).", vreme='".time()."'"))
	    return false;
...
$k=$v['idpro'];
     ...
     $korpa .= '
	      <tr>
	        <td width="85%" colspan="3" style="background:#fff; padding:3px 5px 3px 5px">Dostava</td>
...
$ukupno_sve=$ukupno_rsd=$pri1['ukupnosve'];
    ...
    <span style="color:#CC0000;"><b>'.number_format($ukupno_sve, 2, ',', '.').' rsd</b></span>
          </td>
          </tr>';
...
$korpa .='
    ...
    $korpa .='</td></tr>
    <tr>
      <td>Iznos:<br>'.number_format(round($ukupno_sve,2), 2, ',', '.').' rsd</td>
    </tr>
...
$ukupno_sve=$pri1['ukupnosve'];
	      ...
	      <tr>
	        <td width="85%" colspan="3" style="background:#fff; padding:3px 5px 3px 5px">Ukupno za naplatu</td>
	        <td width="15%" align="center"  style="background:#cccccc; padding:3px 5px 3px 5px">
    <span style="color:#CC0000;"><b>'.number_format(round($ukupno_sve,2), 2, ',', '.').' rsd</b></span>
          </td>
          </tr>';
...
//if($tip=="uspeh" and $br_narudzbine>0) send_email("mail", $settings["email_zaemail"], $settings["from_email"], $settings["from_email"], $naslov, $from_name, $poruka, $aaa1);
...
{
posaljiK("uspeh",2);
mysqli_query($conn, "UPDATE privremena SET payamentid='$PayID', transactionid='$TransID', status=$mdStatus, ProcReturnCode=".safe($ProcReturnCode).", auth='$AutCode', resultcode='$ResCode', trantype='PreAuth', postdate='$PosDate', poslat_email='$poslat_mail', proveren=1  WHERE trackid='$TrckID'");
}
...
elseif($diff>0.3 and $ari1['proveren']==0)
