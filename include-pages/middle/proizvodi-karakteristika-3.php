<?php 
if($settingsc['poz_k3']!="") $pozboja=" style='background:".$settingsc['poz_k3']."'"; else $pozboja="";
if($settingsc['limit_k3']<1) $lim="LIMIT 4"; else $lim="LIMIT ".$settingsc['limit_k3'];
$ztz = mysqli_query($conn, "SELECT p.*, pt.*, p.id as ide
        FROM pro p
        INNER JOIN prol pt ON p.id = pt.id_text
        WHERE pt.lang='$lang' AND p.akt=1 AND p.izdvojeni=1 AND p.prodato=0 AND lager=1 ORDER BY p.pozicija, rand() $lim");
    ...
    <div class="home-module-three hm-1 fix pb-40"<?php echo $pozboja?>>
        <div class="container-fluid">
            <div class="section-title module-three">
                <h3><?php echo $arrwords['karakteristika3']?></h3>
            </div>
...
while($atz1=mysqli_fetch_array($ztz))
 ...
 ?>
 								<div class="col mb-30"data-aos='fade-up' data-aos-duration='800'>
